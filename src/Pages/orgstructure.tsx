...
const OrgChart = () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data && typeof data === 'object') {
          const validatedData: OrgStructure = {
          ...
          setUploadStatus('Loaded from database');
        }
      } else if (response.status === 404) {
        setUploadStatus('No data in database, using default data');
      } else {
        setUploadStatus(`Server error: ${response.status}, using default data`);
      }
    } catch (fetchError) {
      console.error('Fetch error:', fetchError);
      setUploadStatus(`Connection error, using default data`);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:8000/api/org/structure', {
        method: 'POST',
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 py-6 px-4">
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        <h2 className="text-xl font-bold text-gray-800 mb-4">Organization Structure</h2>

        <div className="flex flex-col sm:flex-row items-center space-y-4 sm:space-y-0 sm:space-x-4 mb-4">
          <label htmlFor="excel-upload" className="px-6 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 transition cursor-pointer flex items-center space-x-2">
            <Upload className="w-5 h-5" />
            <span>{isLoading ? 'Processing...' : 'Upload Excel File'}</span>
          </label>
          <input id="excel-upload" type="file" accept=".xlsx,.xls" onChange={handleFileUpload} className="hidden" disabled={isLoading} />

          <button onClick={loadOrgData} disabled={isLoading} className="px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition flex items-center space-x-2">
            <RefreshCw className={`w-5 h-5 ${isLoading ? 'animate-spin' : ''}`} />
            <span>Refresh</span>
          </button>

          <button onClick={expandAll} className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 transition">
            Expand All
          </button>
          <button onClick={collapseAll} className="px-4 py-2 bg-gray-600 text-white rounded hover:bg-gray-700 transition">
            Collapse All
          </button>
        </div>
        ...
        {uploadStatus && (
          <div className={`p-3 rounded-lg ${
            ...
            uploadStatus.includes('Successfully') ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700'
          }`}>
            {uploadStatus}
          </div>
        )}
        ...
        <Card className="p-6 bg-blue-600 text-white shadow-xl rounded-lg">
          <div className="flex flex-col items-center space-y-3">
            ...
            <div className="text-center">
              ...
              <p className="text-white opacity-90">{orgData.ceo?.title || 'Chief Executive Officer'}</p>
            </div>
          </div>
        </Card>
      </div>
      ...
      <div className="flex justify-end pr-16">
        <Card className="p-4 bg-teal-600 text-white shadow-lg rounded-lg">
          <div className="flex flex-col items-center space-y-2">
            ...
            <div className="text-center">
              <h4 className="font-bold">{orgData.seniorEA?.name || 'Senior EA'}</h4>
              <p className="text-xs text-white opacity-90">{orgData.seniorEA?.title || 'Senior Executive Assistant'}</p>
            </div>
          </div>
        </Card>
      </div>
      ...
      <div className="flex flex-row flex-wrap gap-4 mt-8">
        {branches.map((branch) => (
          <div key={branch.id} className="space-y-2 w-[260px]">
            ...
            <Card
              className={`p-4 ${branch.color || 'bg-orange-500'} text-white shadow-lg rounded-lg cursor-pointer hover:shadow-xl transition-all transform hover:scale-105`}
            ...
            >
              <div className="flex flex-col items-center space-y-3">
                ...
                <div className="text-center">
                  <h4 className="font-bold text-sm">{branch.executive?.name || 'Executive'}</h4>
                  <p className="text-xs opacity-90">
                    {branch.executive?.title || branch.executive?.hierarchyTitle || "Executive"}
                  </p>
                ...
                <div className="text-white">
                  {expandedBranches.has(branch.id) ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
                </div>
              </div>
            </Card>
              ...
              <div className="space-y-3 ml-2">
                ...
                {(!branch.departments || branch.departments.length === 0) && (
                  <Card className="p-3 bg-gray-100 border-l-4 border-l-gray-400 rounded-lg">
                    <p className="text-sm text-gray-600 text-center">No departments found</p>
                  </Card>
                )}
              </div>
            )}
        ...
        {branches.length === 0 && (
          <div className="w-full text-center py-8">
            <p className="text-gray-600">No executive branches found. Please upload employee data.</p>
          </div>
        )}
      </div>
    </div>
